...
function restoreCookies() {
  if (document.cookie.indexOf("scisimulab") >= 0) {
    let cookie = document.cookie
      .split("; ")
      .find((row) => row.startsWith("scisimulab="));
...
function listSimuls(list, filt = {}) {
  ...
  simsfiltred.forEach((simulacao, index) => {
    ...
    <a class="card" href="${simulacao.url}" target="scisimulabWindow">
      ...
      <img src="${
        simulacao.thumb ? simulacao.thumb : unimage
      }" alt="imagem da simulação ${simulacao.name}"/>
      <p>${simulacao.descript}</p>
      <label><strong>Categorias: </strong>${simulacao.categorys.join(
        ", "
      )}</label>
...
btinsert.addEventListener("click", function () {
  ...
  h1message.innerHTML =
    "SIMULAÇÃO ENVIADA PARA ANÁLISE, Agradeçemos sua contribuição";
  setTimeout(function () {
...
function createSlides() {
  ...
  for (let i = 0; i < mysimul.length; i++) {
    ...
    <li onclick="opemwindow(slidersurl[count])">
      ...
      <label class="bullet" for="slide${i}" style="left: ${
      10 + i * 30
    }px"></label>
      <a class="card mycard" href="${mysimul[i].url}" target="scisimulabWindow">
        <div>
            <h3>${mysimul[i].name}</h3>
            <label><strong>Categorias: </strong>${mysimul[i].categorys.join(
              ", "
            )}</label>
            <label><strong>Autor: </strong>${mysimul[i].author}</label>
        </div>
        <img src="${
          mysimul[i].thumb ? mysimul[i].thumb : unimage
        }" alt="imagem da simulação ${mysimul[i].name}"/>
...
function aplicfilters() {
  ...
  const querystrigsearch =
    "?" +
    Object.keys(filt)
      .map(function (key) {
        if (
          encodeURIComponent(filt[key]) != "all" &&
